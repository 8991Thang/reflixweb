...
let nameEdit = localStorage.getItem("nameEdit");
async function renderValue(){
  let dbDataReview = await firebase.firestore().collection("review_moive").where("name","==",nameEdit).get();
  let data = dbDataReview.docs[0].data();
  ...
  des1.value = data.des1;
  des3.value = data.des3;
  des2.value = data.des2;
trailer.value = data.trailer;

  document.querySelector(".img").src = data.img;
  document.querySelector(".background").src = data.imgblur;
  document.querySelector(".img1").src = data.img1;
  document.querySelector(".img2").src = data.img2;
  document.querySelector(".img3").src = data.img3;
}
renderValue();
...
function newReview() {
  formSumit.onsubmit = async function (e) {
    ...
    if(!dataReview.name || !dataReview.time || !dataReview.content || !dataReview.cast || !dataReview.des1 || !dataReview.des2 || !dataReview.des3 || !dataReview.trailer
      ...
      let dbDataReview = await firebase.firestore().collection("review_moive").get();
      await firebase.firestore().collection("review_moive").doc(dbDataReview.docs[0].id).update(dataReview)
      .then(function(){
